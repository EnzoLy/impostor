...
public class NegativeSpaces {
  ...
  private static final HashMap<Integer, String> negativeSpaces = new HashMap<>();

  private static int getMax(int i) {
    var nums =
        negativeSpaces.keySet().stream().filter(ne -> ne > 0 && ne <= i).mapToInt(v -> v).max();
  ...
  public static String get(int number) {
    ...
    var nums =
        negativeSpaces.keySet().stream()
            .filter(nu -> nu > 0 && nu <= fn)
